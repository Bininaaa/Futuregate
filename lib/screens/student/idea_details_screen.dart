    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 18),
          Text(idea.title, style: InnovationHubTypography.title(size: 28)),
          const SizedBox(height: 10),
          if (idea.tagline.trim().isNotEmpty)
            Text(idea.tagline, style: InnovationHubTypography.body(size: 15)),
          if (idea.tagline.trim().isNotEmpty) const SizedBox(height: 14),
          InkWell(
          ...
          OutlinedButton.icon(
            onPressed:
                provider.isInteractionBusy(
                  idea.id,
                  ProjectIdeaInteractionType.spark,
                )
                ? null
                : () => _toggleInteraction(
                    context,
            ...
            icon: Icon(
              idea.isSparkedByCurrentUser
                  ? Icons.flash_on_rounded
                  : Icons.flash_on_outlined,
            ),
            label: Text(
              idea.isSparkedByCurrentUser ? 'Spark sent' : 'Send a Spark',
            ),
          ),
        ],
      ),
