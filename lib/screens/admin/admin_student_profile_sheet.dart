...
class _AdminStudentProfileSheetState extends State<_AdminStudentProfileSheet> {
  ...
  Widget _buildApplicationsSection(
    ...
    final countLabel = switch (snapshot.connectionState) {
      ConnectionState.waiting => _l10n.studentProfileLoadingHistory,
      _ when snapshot.hasError => _l10n.studentProfileHistoryUnavailable,
      _ when items.isEmpty => _l10n.studentProfileNoApplicationsYet,
      _ => _l10n.studentProfileApplicationsCount(items.length),
    };
    ...
    return AdminSurface(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 14),
          _SectionCopy(
            title: _l10n.studentProfileApplicationHistory,
            value: countLabel,
          ),
          if (snapshot.hasError) ...[
            const SizedBox(height: 10),
            Text(
              _l10n.studentProfileSummaryError,
          ...
          SizedBox(
            ...
            child: _DocumentButton(
              label: items.isEmpty
                  ? _l10n.uiViewAllApps
                  : _l10n.studentProfileViewAllAppsCount(items.length),
...
class _AdminStudentApplicationsSheetState
  ...
  void _showApplicationDetails(StudentApplicationItemModel item) {
    final l10n = AppLocalizations.of(context)!;
    final opportunity = item.opportunity;
    final statusLabel = ApplicationStatus.label(item.status, l10n);
    ...
    final description = (opportunity?.description.trim() ?? '').isEmpty
        ? ''
        : DisplayText.capitalizeLeadingLabel(opportunity!.description);
    ...
    final benefits = _detailList(opportunity?.benefits ?? const <String>[]);
            ...
            child: Material(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  AdminSectionHeader(title: l10n.uiApplicationDetails),
                  const SizedBox(height: 12),
                  _DetailRow(
                  ...
                  if (opportunity == null)
                    ...
                    _DetailRow(
                      ...
                      label: l10n.uiType,
                      value: OpportunityType.label(opportunity.type, l10n),
                      preserveValueFormatting: true,
                    ),
                    _DetailRow(
  ...
  String _deadlineLabel(OpportunityModel opportunity, AppLocalizations l10n) {
    ...
    return fallback.isEmpty ? l10n.uiNotSpecified : fallback;
  }
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          Text(
                            snapshot.connectionState == ConnectionState.waiting
                                ? l10n.studentProfileLoadingHistory
                                : items.isEmpty
                                ? l10n.studentProfileNoSubmittedApps
                                : l10n.studentProfileSubmittedAvailable(
                                    items.length,
                                  ),
                            style: AppTypography.product(
                              fontSize: 12.5,
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting)
                      ...
                      AdminEmptyState(
                        ...
                        message: l10n.studentProfileFullHistoryUnavailable,
                      ...
                      AdminEmptyState(
                        ...
                        title: l10n.studentProfileNoApplicationsYetTitle,
                        message: l10n.studentProfileNoApplicationsMessage,
                      )
                    else
  ...
  Widget _buildApplicationCard(StudentApplicationItemModel item) {
    ...
    return Semantics(
      button: true,
      label:
          '${item.title}, ${ApplicationStatus.label(item.status, l10n)}. ${l10n.uiOpenDetails}',
      child: Padding(
        padding: const EdgeInsets.only(bottom: 10),
        child: Material(
...
class _ApplicationDetailHero extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AdminSurface(
      ...
      gradient: AdminPalette.heroGradient(statusColor),
...
class _ApplicationDetailListCard extends StatelessWidget {
  final String title;
  final List<String> items;
  final IconData icon;
  final Color color;

  const _ApplicationDetailListCard({
    required this.title,
    required this.items,
    required this.icon,
    required this.color,
...
String? _opportunityStateLabel(
  StudentApplicationItemModel item,
  AppLocalizations l10n,
) {
  final opportunity = item.opportunity;
  if (opportunity == null) {
    return l10n.studentProfileOpportunityUnavailable;
  }
  if (opportunity.isHidden) {
    return l10n.studentProfileOpportunityHidden;
  }
  if (opportunity.effectiveStatus() == 'closed') {
    return l10n.studentProfileOpportunityClosed;
  }
